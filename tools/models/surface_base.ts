...
function habitat(group: THREE.Object3D, x: number, y: number, rotation: number): void {
  ...
  lathe(site, insulation, [[13.0, 0], [13.6, 1.4], [13.6, 7.4], [12.4, 9.2], [9.0, 10.4]], 16);
  torus(site, metal, 13.2, 0.7, [0, 2.2, 0], [Math.PI / 2, 0, 0], 6, 16);
  torus(site, metal, 13.0, 0.7, [0, 6.6, 0], [Math.PI / 2, 0, 0], 6, 16);
  dome(site, insulation, 9.0, [0, 10.4, 0], [0, 0, 0], 12);
  for (let i = 0; i < 5; i++) {
...
function fuel_plant(group: THREE.Object3D, x: number, y: number): void {
  ...
  for (const side of [-1, 1]) {
    lathe(site, insulation, [[4.2, 0], [4.6, 1.4], [4.6, 24.0], [4.0, 25.6], [1.6, 26.4]], 14, [side * 7.5, 0, 6.4], [Math.PI / 2, 0, 0]);
    for (const along of [-8, 0, 8]) {
      bevelled(site, dark, [5.0, 3.4, 1.6], [side * 7.5, along, 1.7], [0, 0, 0], { radius: 0.35, segments: 1 });
    }
    torus(site, metal, 4.4, 0.4, [side * 7.5, 8.0, 6.4], [0, 0, 0], 5, 12);
    torus(site, metal, 4.4, 0.4, [side * 7.5, 16.0, 6.4], [0, 0, 0], 5, 12);
  }
  for (const at of [[-4.4, 0, 0], [0, 0, 0], [4.4, 0, 0]]) {
    ball(site, insulation, 4.4, [at[0], at[1] - 12.0, 5.0], 10, 6);
  }
  ...
  box(site, copper, [16.0, 1.5, 1.5], [0, 3.4, 1.8]);
...
function comms_mast(group: THREE.Object3D, x: number, y: number): void {
  ...
  truss_box(site, frame, 52.0, 6.0, 6.0, 5, 0.42).position.z = 30.0;
  bevelled(site, hullPaint, [9.0, 9.0, 1.0], [0, 0, 4.0], [0, 0, 0], { radius: 0.5, segments: 1 });
  for (const at of [18.0, 34.0, 50.0]) {
    tube(site, deckPlate, 4.4, 4.4, 0.5, [0, 0, at], 10, [Math.PI / 2, 0, 0]);

  }
  torus(site, frame, 4.0, 0.3, [0, 0, 18.0], [Math.PI / 2, 0, 0], 5, 12);
  torus(site, frame, 4.0, 0.3, [0, 0, 34.0], [Math.PI / 2, 0, 0], 5, 12);
  ...
  site.add(gimbal);
  lathe(gimbal, insulation, [[5.4, 0], [5.0, 0.5], [4.2, 1.6]], 12);

  cylinder(site, metal, 0.8, 0.8, 12.0, [2.4, 0, 58.0], 6);
...
export function build(): THREE.Object3D {
  ...
  for (let i = -2; i <= 2; i++) box(apron, dark, [0.5, 150.0, 0.3], [-4.0 + i * 20.0, 0, 0.16]);
  for (let j = -2; j <= 2; j++) box(apron, dark, [196.0, 0.5, 0.3], [-4.0, j * 20.0, 0.16]);
  ...
  landing_pad_assembly(big, { id: 'pad.A', radius: 21, clamps: 4, lights: 4 });
  const small = new THREE.Group();
  small.position.set(66.0, 46.0, 0);
  group.add(small);
  landing_pad_assembly(small, { id: 'pad.B', radius: 18, clamps: 4, lights: 3, lamp: '#e8c98a' });
  ...
  for (let i = 0; i < 3; i++) box(group, glass, [1.6, 4.2, 1.2], [26.0 + i * 9.0, 30.5 - i * 1.7, 1.5], 0.6);
  ...
  for (let i = 0; i < 7; i++) {
    const at = i * 6.0;
    box(group, metal, [7.5, 1.5, 1.5], [-37.0 + at, -41.0 + at * 0.55, 1.0], 0.55);
  }
  for (let i = 0; i < 2; i++) {
    box(group, i % 2 ? hullPaint : metal, [7.0, 9.0, 5.0], [46.0 + i * 9.0, 23.5, 3.6]);
  }
  ...
  lamps(group, '#b7dfdd', ring, 0.55, 4);
  for (const at of [[-52.0, 58.0], [30.0, -14.0]]) {
