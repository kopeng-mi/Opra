...
export function landing_pad_assembly(parent: THREE.Object3D, options: PadOptions = {}): THREE.Group {
  ...
  tube(group, deckPlate, radius, radius, 1.6, [0, 0, -0.8], 24, [Math.PI / 2, 0, 0]);
  lathe(group, metal, [[radius, 0], [radius + 1.2, -0.5], [radius + 1.2, -1.6], [radius - 3.0, -3.4]], 24, [0, 0, 0], [Math.PI / 2, 0, 0]);
  tube(group, dark, radius - 3.4, radius - 5.0, 2.2, [0, 0, -4.4], 20, [Math.PI / 2, 0, 0]);
  ...
  tube(group, metal, radius - 4.0, radius - 4.0, 0.24, [0, 0, 0.12], 24, [Math.PI / 2, 0, 0]);
  tube(group, metal, 13.6, 13.6, 0.24, [0, 0, 0.12], 24, [Math.PI / 2, 0, 0]);
  // The touchdown gate: a painted circle at 12 m, exactly the gate's distance limit (§3.4).
  tube(group, hazardPaint, 12.2, 12.2, 0.18, [0, 0, 0.16], 24, [Math.PI / 2, 0, 0]);
  for (let i = 0; i < 12; i++) {
  ...
  for (let i = 0; i < clamps; i++) {
    ...
    cylinder(group, metal, 0.55, 0.8, 3.2, [at[0] + Math.sin(angle) * 2.4, at[1] + Math.cos(angle) * 2.4, 1.9], 10);
    cylinder(group, hullPaint, 0.9, 0.9, 5.4, [at[0], at[1], 2.6], 12);
    cylinder(group, dark, 1.5, 1.5, 0.6, [at[0], at[1], 4.4], 8, [Math.PI / 2, 0, 0]);
  }
  ...
  for (let i = 0; i < lights; i++) {
    ...
    strut(group, metal, [x, y, -0.6], [x, y, 4.6], 0.35, 8);
    box(group, dark, [1.6, 1.2, 0.8], [x, y, 5.0], -angle);
    heads.push([x, y, 5.4]);
  }
  const material = lamp_material(lamp);
  for (const head of heads) {
    const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.6, 8, 6), material);
  ...
  strut(group, metal, [-(radius - 7.0), -(radius - 7.0), 0], [-(radius - 7.0), -(radius - 7.0), 9.0], 0.5, 10);
